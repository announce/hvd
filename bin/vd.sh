...
function vd() {
  ...
  __EXTRACT_PUBLIC_METHODS__() {
      cat - | awk '{print "\t" $3}' | grep -v ${SELF} | grep -vE '^\t__[^_]+'
  }

  __USAGE__() {
    echo -e "${SELF} -- ${NAME}
    \nUsage: ${SELF} [sub-command]
    \n[Sub-commands for Local]"
    __LOCAL__ '__METHODS__' | __EXTRACT_PUBLIC_METHODS__
    echo -e "\n[Sub-commands for Remote]"
    __REMOTE__ '__METHODS__' | __EXTRACT_PUBLIC_METHODS__
    echo ''
  }
